{{-- @dump($inputId, $status) --}}
<label class="relative inline-flex items-center cursor-pointer" wire:poll="updateInputStatus">
    <input type="checkbox" id="switch-{{ $inputId }}" name="switch-{{ $inputId }}" class="sr-only peer"
        {{ $status === 1 ? 'checked' : '' }} wire:click="openModal">
    <div
